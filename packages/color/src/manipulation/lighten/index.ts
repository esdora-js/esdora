 ...
 * lighten('#3498db', 20);  // => '#5dade2' (相当于 20%)
 * lighten('invalid', 0.2); // => null
 * ```
 */
 ...
 * ```typescript
 * // 将亮度值固定增加 0.1
 * lighten('#3498db', l => l + 0.1); // => '#5dade2'
 * // 将亮度设置为固定值
 * lighten('#3498db', l => 0.8); // => '#aed6f1'
 * ```
 */
...
export function lighten(color: string | EsdoraColor, amountOrAdjuster: number | LightnessAdjuster): string | null {
  ...
  if (typeof amountOrAdjuster === 'function') {
    ...
    adjuster = (l: number) => l + (1 - l) * ratio
  }

  const lightenColor = adjustLightness(color, adjuster)
  if (!lightenColor) {
    return null
  }
  ...
  if ((lightenColor.alpha ?? 1) < 1) {
    return formatHex8(lightenColor)
  }

  return formatHex(lightenColor)
}
