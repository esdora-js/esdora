 ...
 * @example
 * ```typescript
 * desaturate('#3498db', 0.2); // => '#4a9bd1'
 * desaturate('#3498db', 20);  // => '#4a9bd1' (相当于 20%)
 * desaturate('invalid', 0.2); // => null
 * ```
 */
 ...
 * ```typescript
 * // 将饱和度值固定减少 0.1
 * desaturate('#3498db', s => s - 0.1); // => '#4a9bd1'
 * // 将饱和度设置为固定值
 * desaturate('#3498db', s => 0.3); // => '#6ba3c7'
 * ```
 */
...
export function desaturate(color: string | EsdoraColor, amountOrAdjuster: number | SaturationAdjuster): string | null {
  ...
  if (typeof amountOrAdjuster === 'function') {
    ...
    adjuster = (s: number) => s * (1 - ratio)
  }

  const desaturatedColor = adjustSaturation(color, adjuster)
  if (!desaturatedColor) {
    return null
  }
  ...
  if ((desaturatedColor.alpha ?? 1) < 1) {
    return formatHex8(desaturatedColor)
  }

  return formatHex(desaturatedColor)
}
